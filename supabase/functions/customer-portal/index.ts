...
serve(async (req) => {
  ...
  try {
    ...
    const result = await resolveStripeCustomer(stripe, supaAdmin, user.id, user.email, { createIfMissing: false });
    if (!result) throw new Error("No Stripe customer found for this user");
