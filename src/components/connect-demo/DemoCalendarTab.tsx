...
const DEMO_EVENTS = [
  { id: "1", title: "Pipeline Review — Q3 Targets", type: "meeting", time: "9:00 AM – 10:00 AM", date: today, location: "Zoom", attendees: ["sarah@greenvalley.com", "marcus@techventures.io"], color: "hsl(140 12% 42%)" },
  ...
  { id: "5", title: "Annual Review — Sunrise Properties", type: "coverage_review", time: "2:00 PM – 3:00 PM", date: addDays(today, 1), location: "Zoom", attendees: ["linda@sunriseproperties.com"], color: "hsl(140 12% 42%)" },
  { id: "6", title: "Strategy Session — West Coast Expansion", type: "meeting", time: "10:00 AM – 11:30 AM", date: addDays(today, 2), location: "Board Room", attendees: ["marcus@techventures.io", "amanda@brightfuture.org"], color: "hsl(140 12% 42%)" },
  ...
  { id: "8", title: "Team Standup", type: "meeting", time: "9:00 AM – 9:30 AM", date: addDays(today, 4), location: "Google Meet", attendees: [], color: "hsl(140 12% 42%)" },
];
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-3">
          ...
          {todayEvents.length > 0 && (
            <Badge style={{ background: "hsl(140 12% 42%)", color: "white" }} className="text-[10px]">{todayEvents.length} today</Badge>
          )}
        </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {BOOKING_LINKS.filter(b => b.active).map(b => (
          <button key={b.id} onClick={() => copyLink(b.slug)} className="shrink-0 p-2.5 rounded-lg flex items-center gap-2 transition-all hover:scale-[1.02]" style={{ background: "hsl(240 8% 9%)", border: "1px solid hsl(240 6% 14%)" }}>
            <Link2 className="h-3.5 w-3.5" style={{ color: "hsl(140 12% 58%)" }} />
          ...
          {Object.entries(groupedEvents).sort(([a], [b]) => a.localeCompare(b)).map(([dateKey, events], gi) => {
            ...
            return (
              <div key={dateKey} className="animate-fade-in" style={{ animationDelay: `${gi * 80}ms` }}>
                <h3 className="text-xs font-semibold mb-2 flex items-center gap-2" style={{ color: isToday ? "hsl(140 12% 58%)" : "hsl(240 5% 60%)" }}>
