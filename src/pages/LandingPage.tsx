  ...
  return (
    <div className="min-h-screen bg-[#08080A] text-[#FAFAFA] overflow-x-hidden" style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif" }}>
      ...
      <div className="relative z-[1]">
        ...
        {/* ═══ VERTICALS — Tailored for Every Industry ═══ */}
        <section className="lp-reveal pt-[120px] max-sm:pt-20 pb-[60px] relative" id="verticals" ref={addRevealRef}>
          <div className="max-w-[1200px] mx-auto px-12 max-md:px-8 max-sm:px-5">
            <div className="border-t border-[hsl(140_12%_42%/0.15)] pt-14">
              <div className="mb-10 max-w-[600px]">
                <div className="text-[11px] font-semibold tracking-[0.12em] uppercase mb-4" style={{ color: "hsl(140 12% 58%)" }}>Built for Your Industry</div>
                <h2 className="text-[clamp(28px,3.5vw,44px)] font-bold tracking-[-0.035em] leading-[1.1] text-white mb-4">
                  One Platform.<br />Completely Tailored to You.
                </h2>
                <p className="text-[15px] text-[#71717A] leading-[1.65]">
                  AURA isn't a generic tool with industry labels. When you select your vertical, the entire experience adapts — your lead sources, pipeline stages, outreach templates, AI coaching, and agent capabilities are all configured for how <em>your</em> industry actually works.
                </p>
              </div>
                ...
                {[
                  {
                    ...
                    color: "#01696F",
                    icon: (
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6"><path d="M12 2l8 4v6c0 5.5-3.8 10.7-8 12-4.2-1.3-8-6.5-8-12V6l8-4z" /></svg>
                    ),
                    tagline: "Risk placement, reinvented",
                    points: [
                      "ACORD auto-fill and submission building from intake to binding",
                      "Carrier matching across 200+ appetite guides",
                      "Loss run automation and renewal tracking pipelines",
                      "Leads sourced from business filings, permits, and market triggers",
                    ],
                  },
                  {
                    ...
                    color: "#B87333",
                    icon: (
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6"><path d="M3 21h18" /><path d="M5 21V7l7-4 7 4v14" /><path d="M9 21v-6h6v6" /></svg>
                    ),
                    tagline: "Property deals, accelerated",
                    points: [
                      "Deal sourcing from listing activity, ownership records, and permits",
                      "Property intelligence dashboards with market comps",
                      "Buyer/seller pipeline with transaction stage tracking",
                      "Investor outreach cadences and relationship mapping",
                    ],
                  },
                  {
                    industry: "Mortgage",
                    color: "#6366F1",
                    icon: (
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6"><rect x="2" y="5" width="20" height="14" rx="2" /><path d="M2 10h20" /></svg>
                    ),
                    tagline: "Lending workflows, streamlined",
                    points: [
                      "Pre-qualification lead scoring from credit and property signals",
                      "Automated rate comparison and client follow-up sequences",
                      "Referral partner tracking with real estate agent networks",
                      "Pipeline stages mapped to origination milestones",
                    ],
                  },
                  {
                    ...
                    color: "#C9A84C",
                    icon: (
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6"><path d="M3 3v18h18" /><path d="M7 16l4-8 4 5 5-6" /></svg>
                    ),
                    tagline: "Wealth relationships, unified",
                    points: [
                      "High-net-worth lead sourcing from wealth signals and life events",
                      "Client lifecycle tracking from prospect to AUM growth",
                      "Compliance-aware outreach templates and cadences",
                      "Holistic relationship graph across financial touchpoints",
                    ],
                  },
                  {
                    industry: "Consulting",
                    color: "#8B5CF6",
                    icon: (
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6"><circle cx="12" cy="12" r="3" /><path d="M12 1v4m0 14v4M4.22 4.22l2.83 2.83m9.9 9.9l2.83 2.83M1 12h4m14 0h4M4.22 19.78l2.83-2.83m9.9-9.9l2.83-2.83" /></svg>
                    ),
                    tagline: "Professional services, scaled",
                    points: [
                  ...
                  },
                  {
                    ...
                    color: "hsl(140 12% 42%)",
                    icon: (
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6"><rect x="2" y="7" width="20" height="14" rx="2" /><path d="M16 7V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v2" /></svg>
                    ),
                    tagline: "Your business, your way",
                    points: [
                      "Flexible pipeline and CRM configured to your sales process",
                      "Multi-channel outreach across email, phone, and social",
                      "AI assistant trained on your industry terminology",
                      "Lead marketplace access with full enrichment at the lowest price point",
                    ],
                  },
                ].map((v) => (
                  <div
                    key={v.industry}
                    className="p-6 rounded-2xl border transition-all hover:-translate-y-1 group"
                    style={{ background: "hsl(140 12% 42% / 0.03)", borderColor: "hsl(140 12% 42% / 0.1)" }}
                  >
                    <div className="flex items-center gap-3 mb-4">
                      <div className="p-2.5 rounded-xl" style={{ background: `${v.color}20`, color: v.color }}>
                        {v.icon}
                      </div>
                      <div>
                        <div className="text-[15px] font-bold text-white">{v.industry}</div>
                        <div className="text-[11px] font-medium" style={{ color: v.color }}>{v.tagline}</div>
                      </div>
                    </div>
                    <ul className="space-y-2">
                      {v.points.map((p, i) => (
                        <li key={i} className="flex items-start gap-2 text-[12px] text-[#A1A1AA] leading-[1.55]">
                          <span className="w-1 h-1 rounded-full mt-1.5 shrink-0" style={{ background: v.color }} />
                          {p}
                        </li>
                      ))}
                    </ul>
              ...
              <div className="rounded-2xl p-6 border text-center" style={{ background: "hsl(140 12% 42% / 0.04)", borderColor: "hsl(140 12% 42% / 0.12)" }}>
                <p className="text-[15px] text-[#A1A1AA] leading-[1.65] max-w-[640px] mx-auto">
                  <span className="text-white font-semibold">Select your industry once during signup</span> — and AURA configures your entire workspace: lead sources, pipeline stages, outreach templates, AI assistant context, and agent capabilities. No setup required.
                </p>
                <Link
                  to="/request-access"
                  className="inline-flex items-center gap-2 text-sm font-medium text-[#08080A] px-7 py-3 rounded-[10px] transition-colors no-underline mt-5"
                  style={{ background: "hsl(140 12% 42%)" }}
                  onMouseEnter={(e) => (e.currentTarget.style.background = "hsl(140 12% 52%)")}
                  onMouseLeave={(e) => (e.currentTarget.style.background = "hsl(140 12% 42%)")}
                >
